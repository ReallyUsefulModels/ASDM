...
from Payoff import (
    poisson_log_likelihood_payoff_contribution,
    absolute_error_payoff_contribution,
)
...
class Optimizer(object):
    ...
    def calculate_payoff(self, params):
        ...
        for i in range(len(params)):
            p = list(self.parameters.keys())[
                i
            ]  # taking advantage of the ordered dict since Python 3.7
        ...
        for ov, ts in self.output_vars.items():
            ...
            for t in range(int(self.time_units / self.dt)):
                payoff_infected_t = self.payoff_function(
                    ts[t], sim[t], weight=1.0 / len(params)
                )
        ...
        if self.inference_method == "MLE":
            ...
            self.inference_method == "prior"
        ):  # only sample the prior, no data include, for testing
            payoff = log_prior
        else:
            raise exception(
                "Error: Estimate method {} not defined".format(self.inference_method)
            )
