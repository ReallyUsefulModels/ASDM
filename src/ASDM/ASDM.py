...
class Parser(object):
    def __init__(self):
        ...
        self.functions = (
            {  # use lookahead (?=\() to ensure only match INIT( not INITIAL
                "MIN": r"MIN(?=\()",
                "MAX": r"MAX(?=\()",
                "RBINOM": r"RBINOM(?=\()",
                "INIT": r"INIT(?=\()",
        ...
        while len(s) > 0:
            ...
            for type_name, type_regex in (
                ...
                if m:
                    ...
                    if (
                        item[0] == '"' and item[-1] == '"'
                    ):  # strip quotation marks from matched string
                        item = item[1:-1]
                    if type_name == "ABSOLUTENAME":
                        type_name = "NAME"
                    if (
                        type_name == "NUMBER" and item[0] == "-"
                    ):  # fix the -1: negative 1 vs minus 1 problem
    ...
    def parse(self, string, verbose=False, max_iter=100):
        ...
        if len(items) == 1:
            ...
            graph.add_node(self.node_id, operator=operator, operands=items)
                ...
                for i in range(len(items)):  # loop over all positions for this pattern
                    ...
                    for pattern, func in pattern_set.items():
                        ...
                        if len(items) - i >= pattern_len:
                            ...
                            for j in range(
                                pattern_len
                            ):  # matching pattern at this position
                                if pattern[j] == items[i + j][0]:  # exact match
                                    ...
                                    if pattern[j] == "DOT+":  # fuzzy match
                                        ...
                                        try:
                                            next_to_match = pattern[
                                                j + 1
                                            ]  # it is pattern[j+1] that matters
                            ...
                            if matched:
                                ...
                                if verbose:
                                    print(
                                        self.HEAD,
                                        "Found {} at {}".format(pattern, matched_items),
                                    )
                                operands = []
...
class Solver(object):
    def __init__(
        ...
        def rbinom(n, p):
            ...
            return float(
                s
            )  # TODO: something is wrong here - the dimension of s goes high like [[[[30]]]] if not float()ed.
    ...
    def calculate_node(
        ...
        if verbose:
            print(
                "\t" * self.id_level + self.HEAD + " [ " + var_name + " ] ",
                "processing node {} on subscript {}:".format(node_id, subscript),
            )
        ...
        if type(parsed_equation) is dict:
            ...
            if operator[0] == "IS":
                ...
                operator[0] == "SPAREN"
            ):  # TODO this part is too dynamic, therefore can be slow. Need to resolve this when compiling.
                ...
                if len(operands) == 1:  # only var_name; no subscript is specified
                    ...
                    if (
                        subscript
                    ):  # subscript is explicitly specified; like a[Element_1] or a[Dimension_1]
                        if verbose:
                            print(
                                "\t" * self.id_level
                                + self.HEAD
                        ...
                        for i in range(len(operands_containing_subscript)):
                            if (
                                operands_containing_subscript[i][1]
                                in self.dimension_elements.keys()
                            ):  # it's sth like Dimension_1
                                # if verbose:
                                #     print('\t'*self.id_level+self.HEAD+' [ '+var_name+' ] ', 'a1.2.0.1')
                                subscript_from_operands.append(
                                    subscript[i]
                                )  # take the element from arg subscript in the same position to replace Dimension_1
                        ...
                        value = self.name_space[var_name][
                            subscript_from_operands
                        ]  # try if subscript is Element_1
                        ...
                        subscript = tuple(
                            operand[1] for operand in operands[1:]
                ...
                operator[0] in self.built_in_functions.keys()
            ):  # plus, minus, con, etc.
                if verbose:
                    print(
                        "\t" * self.id_level + self.HEAD + " [ " + var_name + " ] ",
                        "operator v7 Built-in operator:",
                        operator,
                        operands,
                    )
                ...
                if func_name == "INIT":
                    ...
                    # expr value
                    expr_value = self.calculate_node(
                        parsed_equation=parsed_equation,
                        node_id=operands[0][2],
                        verbose=verbose,
                    )
                    ...
                    if (
                        len(operands) == 2
                    ):  # there's no initial value specified -> use the delyed expr's initial value
                        init_value = self.time_expr_register[tuple(operands[0])][0]
                    elif len(operands) == 3:  # there's an initial value specified
                        init_value = self.calculate_node(
                            parsed_equation=parsed_equation,
                            node_id=operands[2][2],
                            verbose=verbose,
                        )
                    else:
                        raise Exception(
                            "Invalid initial value for DELAY in operands {}".format(
                                operands
                            )
                        )

                    # delay time
                    delay_time = self.calculate_node(
                        parsed_equation=parsed_equation,
                        node_id=operands[1][2],
                        verbose=verbose,
                    )
                    if delay_time > (
                        self.sim_specs["current_time"] - self.sim_specs["initial_time"]
                    ):  # (- initial_time) because simulation might not start from time 0
                        value = init_value
                    else:
                    ...
                    if tuple(operands[0]) not in self.time_expr_register.keys():
                        ...
                        for i in range(order):
                            self.time_expr_register[tuple(operands[0])].append(
                                delay_time / order * init_value
                            )
                    # outflows
                    outflows = list()
                    for i in range(order):
                        outflows.append(
                            self.time_expr_register[tuple(operands[0])][i]
                            / (delay_time / order)
                            * self.sim_specs["dt"]
                        )
                        self.time_expr_register[tuple(operands[0])][i] -= outflows[i]
                    # inflows
                    self.time_expr_register[tuple(operands[0])][0] += (
                        expr_value * self.sim_specs["dt"]
                    )
                    for i in range(1, order):
                        self.time_expr_register[tuple(operands[0])][i] += outflows[
                            i - 1
                        ]
                    ...
                    # historical time
                    historical_time = self.calculate_node(
                        parsed_equation=parsed_equation,
                        node_id=operands[1][2],
                        verbose=verbose,
                    )
                    if historical_time > self.sim_specs["current_time"]:
                        ...
                        historical_steps = (
                            historical_time - self.sim_specs["initial_time"]
                        ) / self.sim_specs["dt"]
                        value = self.time_expr_register[tuple(operands[0])][
                            int(historical_steps)
                        ]
                    ...
                    raise Exception(
                        "Unknown time-related operator {}".format(operator[0])
                    )
                if verbose:
                    print(
                        "\t" * self.id_level + self.HEAD + " [ " + var_name + " ] ",
                        "v9 Time-related Func:",
                        value,
                    )
                ...
                if func_name == "LOOKUP":
                    ...
                    look_up_func_name = parsed_equation.nodes[look_up_func_node][
                        "operands"
                    ][0][1]
                    look_up_func = self.graph_functions[look_up_func_name]
                    input_value = self.calculate_node(
                        parsed_equation=parsed_equation,
                        node_id=operands[1][2],
                        verbose=verbose,
                    )
        ...
        if verbose:
            print(
                "\t" * self.id_level + self.HEAD + " [ " + var_name + " ] ",
                "value for {} {}".format(node_id, subscript),
                value,
            )
        ...
        if subscript is None:
            if type(value) is dict:
                raise Exception(
                    "Value cannot be dict if subscript is None. Var: {}".format(
                        var_name
                    )
                )
            else:
...
class GraphFunc(object):
    ...
    def __call__(self, input):
        ...
        output = float(
            self.interp_func(input)
        )  # the output (like array([1.])) needs to be converted to float to avoid dimension explosion
        return output
    ...
    def initialize(self, length, value, leak_fraction=None):
        ...
        if leak_fraction is None or leak_fraction == 0:
            ...
            self.output = self.total / (
                self.length_steps
                + (n_leak * self.leak_fraction)
                / ((1 - self.leak_fraction) * self.length_steps)
            )
            # print('Conveyor Output:', output)
            leak = self.output * (
                self.leak_fraction / ((1 - self.leak_fraction) * self.length_steps)
            )
...
class Structure(object):
    ...
    def __init__(self, from_xmile=None):
        ...
        self.stock_shadow_values = (
            dict()
        )  # temporary device to store in/out flows' effect on stocks.
        self.time_slice = dict()
        self.full_result = dict()
        self.full_result_flattened = dict()

        # env variables
        self.env_variables = {"TIME": 0}
        ...
        if from_xmile is not None:
            ...
            if xmile_path.exists():
                ...
                try:
                    ...
                    self.dimension_elements.update(
                        dims
                    )  # need to use update here to do the 'True' assignment
                ...
                def read_graph_func(var):
                    ...
                    if xscale is None and xpts is None:
                        raise Exception(
                            "GraphFunc: xscale and xpts cannot both be None."
                        )
                    ...
                    equation = GraphFunc(
                        yscale=yscale, ypts=ypts, xscale=xscale, xpts=xpts
                    )
                    return equation
                    ...
                    if var.find("dimensions"):
                        ...
                        if len(var_elements) != 0:
                            for var_element in var_elements:
                                ...
                                elements = self.process_subscript(
    ...
    def replace_element_equation(self, name, new_equation):
        if type(new_equation) is str:
            ...
            raise Exception(
                "Unsupported new equation {} type {}".format(
                    new_equation, type(new_equation)
                )
            )
    ...
    def parse_1(self, var, equation):
        if type(equation) is GraphFunc:
            ...
            self.graph_functions_renamed[
                gfunc_name
            ] = equation  # just for length ... for now
            self.graph_functions[var] = equation
            self.parser.functions.update(
                {gfunc_name: gfunc_name + "(?=\()"}
            )  # make name var also a function name and add it to the parser
            self.parser.patterns_custom_func.update(
                {
                    gfunc_name
                    + "__LPAREN__FUNC__RPAREN": {  # GraphFunc(var, expr, etc.)
            ...
            equation = (
                gfunc_name + "(" + equation.eqn + ")"
            )  # make equation into form like var(eqn),
            ...
            type(equation) is Conveyor
        ):  # TODO we should also consider arrayed conveyors
            self.conveyors[var] = {
            ...
            equation_length = (
                equation.length_time_units
            )  # this is the equation for its length
            parsed_equation_len = self.parser.parse(equation_length)

            equation_init_value = (
                equation.equation
            )  # this is the equation for its initial value
    ...
    def is_dependent(self, var1, var2):
        ...
        dependent = False
        parsed_equation_var2 = (
            self.stock_equations_parsed
            | self.flow_equations_parsed
            | self.aux_equations_parsed
        )[var2]
        leafs = [
            x
            for x in parsed_equation_var2.nodes()
            if parsed_equation_var2.out_degree(x) == 0
        ]
        for leaf in leafs:
            if parsed_equation_var2.nodes[leaf]["operator"][0] in ["EQUALS", "SPAREN"]:
                ...
                if (
                    operands[0][0] == "NUMBER"
                ):  # if 'NUMBER' then pass, as numbers (e.g. 100) do not have a node
                    pass
                    ...
                    operands[0][0] == "FUNC"
                ):  # this refers to a subscripted variable like 'a[ele1]'
                    # need to find that 'SPAREN' node
                    var_dependent_node_id = operands[0][2]
                    var_dependent = parsed_equation_var2.nodes[var_dependent_node_id][
                        "operands"
                    ][0][1]
                    if var_dependent == var1:
                        dependent = True
    ...
    def calculate_dependents(self, parsed_equation):
        leafs = [
            x for x in parsed_equation.nodes() if parsed_equation.out_degree(x) == 0
        ]
        for leaf in leafs:
            # print('i4.1')
            if parsed_equation.nodes[leaf]["operator"][0] in ["EQUALS", "SPAREN"]:
                ...
                if (
                    ...
                    # need to find that 'SPAREN' node
                    var_dependent_node_id = operands[0][2]
                    var_dependent = parsed_equation.nodes[var_dependent_node_id][
                        "operands"
                    ][0][1]
                    # print('var_dependent2', var_dependent)
                    self.calculate_variable_dynamic(var=var_dependent)

    def calculate_variable_dynamic(
        self,
        var,
        subscript=None,
        verbose=False,
        ...
        if var == "TIME":
            ...
            if var in self.conveyors:
                ...
                if not (conveyor_init or conveyor_len):
                    if not self.conveyors[var]["conveyor"].is_initialized:
                        ...
                        self.calculate_variable_dynamic(
                            var=var,
                            subscript=subscript,
                            verbose=verbose,
                            conveyor_len=True,
                        ...
                        if len(leak_flows) == 0:
                            ...
                            for leak_flow in leak_flows.keys():
                                ...
                                leak_fraction = self.conveyors[var]["leakflow"][
                                    leak_flow
                                ]  # TODO multiple leakflows
                        self.conveyors[var]["conveyor"].initialize(
                            length_steps, conveyor_init_value, leak_fraction
                        )
                    ...
                    if var not in self.stock_shadow_values:
                        ...
                        # leak
                        for leak_flow, leak_fraction in self.conveyors[var][
                            "leakflow"
                        ].items():
                            if leak_flow not in self.name_space:
                                # print('    Calculating leakflow {} for {}'.format(leak_flow, var))
                                leaked_value = self.conveyors[var][
                                    "conveyor"
                                ].leak_linear()
                                self.name_space[leak_flow] = (
                                    leaked_value / self.sim_specs["dt"]
                                )  # TODO: we should also consider when leak flows are subscripted
                        # out
                        for outputflow in self.conveyors[var]["outputflow"]:
                            if outputflow not in self.name_space:
                                ...
                                outflow_value = self.conveyors[var][
                                    "conveyor"
                                ].outflow()
                                self.name_space[outputflow] = (
                                    outflow_value / self.sim_specs["dt"]
                                )
                        # print("    Name space2:", self.name_space)
                        self.stock_shadow_values[var] = self.conveyors[var][
                            "conveyor"
                        ].level()
                    ...
                    self.conveyors[var]["len"] = self.solver.calculate_node(
                        parsed_equation=parsed_equation, verbose=verbose, var_name=var
                    )
                    ...
                    self.calculate_dependents(parsed_equation=parsed_equation)
                    self.conveyors[var]["val"] = self.solver.calculate_node(
                        parsed_equation=parsed_equation, verbose=verbose, var_name=var
                    )
            ...
            # B: var is a normal stock
                    ...
                    if type(parsed_equation) is dict:
                        for k, sub_parsed_equation in parsed_equation.items():
                            ...
                            self.calculate_dependents(
                                parsed_equation=sub_parsed_equation
                            )
                            value = self.solver.calculate_node(
                                parsed_equation=sub_parsed_equation,
                                subscript=k,
                                verbose=verbose,
                                var_name=var,
                ...
                if var not in self.stock_shadow_values:
                    ...
                    if (
                        var in self.stock_flows
                    ):  # some stocks are not connected to any flow
                            ...
                            for in_flow in in_flows:
                                ...
                                if var in self.stock_non_negative:
                                    if type(self.name_space[in_flow]) is not dict:
                                        if (
                                            self.stock_shadow_values[var]
                                            + self.name_space[in_flow]
                                            * self.sim_specs["dt"]
                                            < 0
                                        ):
                                            self.name_space[in_flow] = (
                                        ...
                                        for sub, subval in self.name_space[
                                            in_flow
                                        ].items():
                                            if (
                                                self.stock_shadow_values[var][sub]
                                                + subval * self.sim_specs["dt"]
                                                < 0
                                            ):
                                                self.name_space[in_flow][sub] = (
                                                    self.stock_shadow_values[var][sub]
                                                    * -1
                                                    / self.sim_specs["dt"]
                                                )
            ...
            # C: var is a flow
                ...
                if var in self.leak_conveyors:
                    if not leak_frac:
                        ...
                        if (
                            var not in self.name_space
                        ):  # the leak_flow is not calculated, which means the conveyor has not been initialised
                            self.calculate_variable_dynamic(
                                var=self.leak_conveyors[var], subscript=subscript
                            )
                    else:
                        ...
                        self.calculate_dependents(parsed_equation=parsed_equation)
                        self.conveyors[self.leak_conveyors[var]]["leakflow"][
                            var
                        ] = self.solver.calculate_node(
                            parsed_equation=parsed_equation,
                            verbose=verbose,
                            var_name=var,
                        )
                    ...
                    if (
                        var not in self.name_space
                    ):  # the outflow is not calculated, which means the conveyor has not been initialised
                        self.calculate_variable_dynamic(
                            var=self.outflow_conveyors[var], subscript=subscript
                        )
                    ...
                    if var not in self.name_space:
                        if type(parsed_equation) is dict:
                            for k, sub_parsed_equaton in parsed_equation.items():
                                # self.calculate_variable_dynamic(var=var, subscript=k)
                                self.calculate_dependents(
                                    parsed_equation=sub_parsed_equaton
                                )
                                value = self.solver.calculate_node(
                                    parsed_equation=sub_parsed_equaton,
                                    subscript=k,
    ...
    def simulate(
        self, time=None, dt=None, dynamic=True, verbose=False, debug_against=None
    ):
            ...
            if debug is not False:
                ...
                if not all_var_validated:
                    ...
                    for invalid_var in invalid_vars:
                        print(
                            "Result of {} at Time [{}], Step [{}] invalid".format(
                                invalid_var,
                                self.sim_specs["current_time"],
                                self.current_step,
                            )
                        )
                        ...
                        if type(asdm_result) is dict:
                            for sub, subval in asdm_result.items():
                                print(
                                    "Stella result:",
                                    debug[
                                        self.var_name_to_csv_entry(
                                            invalid_var + "[{}]".format(", ".join(sub))
                                        )
                                    ],
                                )
                        else:
                            print(
                                "Stella result:",
                                debug[self.var_name_to_csv_entry(invalid_var)],
                            )
                        print("\n")
    ...
    def trace_error(self, var_with_error, sub=None):
        ...
        print(
            self.debug_level * "\t", "ASDM value    :", self.name_space[var_with_error]
        )
        print(
            self.debug_level * "\t",
            "Expected value:",
            self.df_debug_against.iloc[self.current_step][
                self.var_name_to_csv_entry(var_with_error)
            ],
        )
            ...
            if parsed_equation.nodes[leaf]["operator"][0] in ["EQUALS", "SPAREN"]:
                ...
                if operands[0][0] == "NUMBER":
                    ...
                    print(
                        self.debug_level * "\t",
                        "   ASDM value    :",
                        self.name_space[var_dependent],
                    )
                    print(
                        self.debug_level * "\t",
                        "   Expected value:",
                        self.df_debug_against.iloc[self.current_step][
                            self.var_name_to_csv_entry(var_dependent)
                        ],
                    )
        ...
        if var_with_error in self.flow_stocks:
            ...
            for direction, connected_stock in connected_stocks.items():
                print(
                    self.debug_level * "\t",
                    "-- Connected stock: {:<4} {}".format(direction, connected_stock),
                )
                print(
                    self.debug_level * "\t",
                    "   ASDM value    :",
                    self.name_space[connected_stock],
                )
                print(
                    self.debug_level * "\t",
                    "   Expected value:",
                    self.df_debug_against.iloc[self.current_step][
                        self.var_name_to_csv_entry(connected_stock)
                    ],
                )
    ...
    def var_name_to_csv_entry(self, var, sub=None):
        ...
        if (
            series_key[0].isdigit() or series_key[-1] == ")"
        ):  # 1 day -> "1 day", a(b)-> "a(b)"
    ...
    def get_element_simulation_result(self, name, subscript=None):
        if not subscript:
            if (
                type(
                    (self.stock_equations | self.flow_equations | self.aux_equations)[
                        name
                    ]
                )
                is dict
    ...
    def display_results(self, variables=None):
        if type(variables) is list and len(variables) == 0:
            variables = list(
                (self.stock_equations | self.flow_equations | self.aux_equations).keys()
            )
