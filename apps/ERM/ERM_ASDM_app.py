...
# Variables
st.subheader(
    "Slide the Sliders to the left to Vary The starting proportion of patients waiting over 24mths for treatment and the time to simulate over in weeks"
)
...
long_wait = st.sidebar.slider(
    "Waiting over 24mths for treatment at model start", 1, 1000, 150
)

# Add a slider to the sidebar:
shorter_wait = st.sidebar.slider(
    "Waiting 6 to 12mths for treatment at model start", 1, 1000, 400
)

# Edit Stock
model.add_stock(
    name="Waiting over 24mths for treatment",
    equation={"nosubscript": str(long_wait)},
    x=0,
    y=0,
    non_negative=True,
)
model.add_stock(
    name="Waiting 6 to 12mths for treatment",
    equation={"nosubscript": str(shorter_wait)},
    x=0,
    y=0,
    non_negative=True,
)
...
df_backlog = df_outcome[
    [
        "Total waiting for diagnostics or treatment",
        "Waiting 6 to 12mths for treatment",
        "Waiting 12 to 24mths for treatment",
        "Waiting over 24mths for treatment",
    ]
]
